...
-- Shared helper-grid drawing, used by both the engine mapgen script
-- (mapgen.lua) and the pure-Lua mapgen script (lua_mapgen.lua).
-- This file is registered as a mapgen script before either of those so
-- that draw_helper_grid() is available to whichever one runs next.
...
-- Replaces non-air nodes on the six faces of the mapchunk [pos_min, pos_max]
-- with their corresponding colorized edge-marker nodes.
-- Reads and writes the VoxelManip data in-place.
-- Does nothing if the helper-grid setting is disabled.
function draw_helper_grid(vm, pos_min, pos_max)
